...
<body class="layout-fixed sidebar-expand-lg bg-body-tertiary">
            ...
            <form action="customer_save.php" method="POST" class="row g-3" novalidate>
              ...
              <div class="row mb-3">
                <div class="col-md-12 text-end">
                  <a href="customer?token=<?php echo $_SESSION['csrf_token']?>" class="btn btn-outline-secondary">
  ...
  // 🔹 Load PPP Secret dari API
      function loadSecrets(selectedName = "") {
        $.getJSON("customer_api.php?action=secrets", function(res){
          if(res.success){
            let html = `<option value="">-- Pilih Secret --</option>`;
            res.data.forEach(p=>{
              html += `<option value="${p.name}" ${p.name == selectedName ? "selected" : ""}>${p.name} (${p.service})</option>`;
            });
            $("#ppp_secret").html(html).trigger('change');
          } else {
            $("#ppp_secret").html(`<option value="">Gagal memuat secret</option>`);
          }
        }).fail(function(){
          $("#ppp_secret").html(`<option value="">Error koneksi API</option>`);
        });
      }
